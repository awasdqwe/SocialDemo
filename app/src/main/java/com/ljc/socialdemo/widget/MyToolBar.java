...
public class MyToolBar extends Toolbar {
    ...
    private FrameLayout titleLeftFrame;
...
//        setNavigationIcon(R.drawable.ic_back_normal);
        ...
        showBack = typedArr.getBoolean(R.styleable.MyToolBar_showBack, true);
        ...
        int titleLeftDrawableRes = typedArr.getResourceId(
                R.styleable.MyToolBar_titleLeftDrawable, 0);
        if (titleLeftDrawableRes != 0)
            setTitleLeftDrawable(titleLeftDrawableRes);
...
//                        , Gravity.TOP | Gravity.LEFT);
        ...
        titleLeftTv.setLayoutParams(tlpLeft);
        titleLeftFrame.addView(titleLeftTv);
    }
...
//                        , Gravity.TOP | Gravity.CENTER_HORIZONTAL);
    ...
    private void addLeftIv() {
        if (titleLeftIv != null) {
            return;
        }
        titleLeftIv = new ImageView(getContext());
        /*Toolbar.LayoutParams tlpRight =
                new Toolbar.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT
                        , Gravity.TOP | Gravity.RIGHT);
        tlpRight.rightMargin = UIUtils.getDimens(R.dimen.dp16);
        titleLeftIv.setLayoutParams(tlpRight);*/
        FrameLayout.LayoutParams flp = new FrameLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT);
        flp.gravity = Gravity.CENTER_VERTICAL;
        titleLeftFrame.addView(titleLeftIv, flp);
    }
    ...
    public MyToolBar setTitleLeftText(CharSequence titleLeftText) {
        ...
        if (titleLeftIv != null) titleLeftIv.setVisibility(View.GONE);
        return this;
    }
    ...
    public void clearLeft() {
        if (titleLeftFrame == null) return;
        titleLeftFrame.setOnClickListener(null);
        if (titleLeftTv != null)
    ...
    public MyToolBar setTitleRightText(String titleRightText, int color) {
        initRightFrame();
        addRightTv();
        titleRightTv.setText(titleRightText);
        titleRightTv.setTextColor(ContextCompat.getColor(getContext(), color));
        titleRightTv.setVisibility(View.VISIBLE);
        if (titleRightIv != null) titleRightIv.setVisibility(View.GONE);
        return this;
    }

    public MyToolBar setTitleRightText(String titleRightText, int color, int background) {
        initRightFrame();
        addRightTv();
        titleRightTv.setText(titleRightText);
        titleRightTv.setTextColor(ContextCompat.getColor(getContext(), color));
        titleRightTv.setBackground(ContextCompat.getDrawable(getContext(), background));
        titleRightTv.setVisibility(View.VISIBLE);
        if (titleRightIv != null) titleRightIv.setVisibility(View.GONE);
        return this;
    }
    ...
    private void initLeftFrame() {
        if (titleLeftFrame != null) return;
        titleLeftFrame = new FrameLayout(getContext());

        titleLeftFrame.setMinimumHeight(MyToolBar.this.getHeight());
        LayoutParams tlpRight =
                new LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT
//                        , Gravity.TOP | Gravity.RIGHT);
                        , Gravity.CENTER_VERTICAL | Gravity.LEFT);
        tlpRight.leftMargin = UIUtils.getDimens(R.dimen.dp16);
        titleLeftFrame.setLayoutParams(tlpRight);
        addView(titleLeftFrame);
        if (!showBack){
            titleLeftFrame.setVisibility(GONE);
        }
    }
...
//                        , Gravity.TOP | Gravity.RIGHT);
    ...
    public MyToolBar setTitleLeftDrawable(int drawResId) {
        initLeftFrame();
        addLeftIv();
        titleLeftIv.setImageResource(drawResId);
        titleLeftIv.setVisibility(View.VISIBLE);
        if (titleLeftTv != null) {//文字给优先级
            titleLeftIv.setVisibility(View.GONE);
        }
        return this;
    }
    ...
    public MyToolBar setTitleLeftClickListener(OnClickListener onClickListener) {
        initLeftFrame();
        titleLeftFrame.setOnClickListener(onClickListener);
//        if (titleLeftTv != null) {
...
//        }
    ...
    public FrameLayout getTitleLeftFrame() {
        initLeftFrame();
        return titleLeftFrame;
    }
    ...
    public ImageView getTitleLeftIv() {//其实不需要初始化，使用时应该判空。上面需要改一下
        return titleLeftIv;
    }
